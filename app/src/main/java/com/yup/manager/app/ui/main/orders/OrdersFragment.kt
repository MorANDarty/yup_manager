...
class OrdersFragment : Fragment(), OrdersCallback {
    ...
    override fun onCallback(order: Order) {
        ...
        bundle.putString("user_name_and_surname", order.owner.name + " " + order.owner.surname)
        bundle.putString("user_phone", order.owner.phone)
        bundle.putString("user_avatar", order.owner.picture)
